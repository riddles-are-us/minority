...
use zkwasm_rest_convention::TransactionData;
use zkwasm_rest_convention::clear_events;
use zkwasm_rest_convention::Command;
use zkwasm_rest_convention::CommandHandler;
use crate::error::*;
...
pub struct Transaction (TransactionData<Activity>);
...
impl Transaction {
    ...
    pub fn decode(params: &[u64]) -> Self {
        let tdata = TransactionData::<Activity>::decode(params);
        Transaction (tdata)
    }
    ...
    pub fn create_player(pkey: &[u64; 4]) -> Result<(), u32> {
        let round = GLOBAL_STATE.0.borrow().round;
        let player = GamePlayer::get(pkey);
        match player {
            Some(_) => Err(ERROR_PLAYER_ALREADY_EXIST),
            None => {
                let mut player = GamePlayer::new(pkey);
    ...
    pub fn process(&self, pkey: &[u64; 4], rand: &[u64; 4]) -> Vec<u64> {
        ...
        let e = match &self.0.command {
            ...
            Command::InstallPlayer => Self::create_player(pkey)
                .map_or_else(|e| e, |_| 0),
            Command::Withdraw(cmd) => cmd.handle::<PlayerData>(&pid, self.0.nonce, rand, counter)
                .map_or_else(|e| e, |_| 0),
            Command::Deposit(cmd) => {
                unsafe { require(*pkey == *ADMIN_PUBKEY) };
                cmd.handle::<PlayerData>(&pid, self.0.nonce, rand, counter)
                    .map_or_else(|e| e, |_| 0)
            },
            Command::Activity(cmd) => cmd.handle::<PlayerData>(&pid, self.0.nonce, rand, counter)
                .map_or_else(|e| e, |_| 0),
        };
        let round = GLOBAL_STATE.0.borrow().round;
